...
import { Agentsstatus } from '../../Commonapicall/AgentsSupplierapicall/Agentsapis';
import { toast } from 'react-toastify';

interface StatusAgentsPopupProps {
    closePopup: () => void;
    AgentData: {
        id: number;
        name?: string;  // optional if you want to display the name
...
export const StatusAgentsPopup: React.FC<StatusAgentsPopupProps> = ({
    ...
    const handleConfirm = async () => {
        setError(null);
        try {
            const newStatus = !AgentData.currentStatus;
            const response = await Agentsstatus(
            ...
            <div className="container mx-auto">
                <div className="relative bg-white rounded-[5px] w-4/12 mx-auto px-5 py-5">
                    <div className="relative mb-10">
                        <h2 className="text-2xl text-armsBlack font-semibold pb-3 border-b-2 border-armsgrey ">Agents/supplier Status</h2>
                    ...
                    <div className="text-center">
                        ...
                        {/* <p className="text-lg text-armsBlack">
                            Are you sure you want to change status for <strong>{AgentData.name}</strong> from 
                            <strong> {AgentData.currentStatus ? 'Active' : 'Inactive'}</strong> to 
                            <strong> {!AgentData.currentStatus ? 'Active' : 'Inactive'}</strong>?
                        </p> */}
