...
import { ClientEnquiryStatus } from '../../Commonapicall/ClientEnquiryapicall/ClientEnquiryapis';
;

interface StatusClientEnquiryPopupProps {
    closePopup: () => void;
    ClientEnquiryData: {
        id: number;
        name: string;
...
export const StatusClientEnquiryPopup: React.FC<StatusClientEnquiryPopupProps> = ({
    ...
    const handleConfirm = async () => {
        ...
        try {
            const newStatus = !ClientEnquiryData.currentStatus;
            const response = await ClientEnquiryStatus(
                ClientEnquiryData.id.toString(),
                newStatus ? '1' : '0'
            );
            closePopup();
            ...
            <div className="container mx-auto">
                <div className="relative bg-white rounded-[5px] w-4/12 mx-auto px-5 py-5">
                    <div className="relative mb-10">
                        <h2 className="text-2xl text-armsBlack font-semibold pb-3 border-b-2 border-armsgrey ">Client Enquiry Status</h2>
