...
import { Manpowerstatus } from '../../Commonapicall/ManpowerSupplyapicall/Manpowerapis';
import { toast } from 'react-toastify';

interface StatusManpowerPopupProps {
    closePopup: () => void;
    ManpowerData: {
        id: number;
        name: string;
...
export const StatusManpowerPopup: React.FC<StatusManpowerPopupProps> = ({
    ...
    const handleConfirm = async () => {
        setError(null);
        try {
            const newStatus = !ManpowerData.currentStatus;
            const response = await Manpowerstatus(
                ManpowerData.id.toString(),
                newStatus ? '1' : '0'
            );
            closePopup();
            ...
            <div className="container mx-auto">
                <div className="relative bg-white rounded-[5px] w-4/12 mx-auto px-5 py-5">
                    <div className="relative mb-10">
                        <h2 className="text-2xl text-armsBlack font-semibold pb-3 border-b-2 border-armsgrey ">Manpower Supply Status</h2>
